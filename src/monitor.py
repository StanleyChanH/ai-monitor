...
class MonitorPipeline:
    ...
    async def _run_inference(self, frame: ProcessedFrame) -> None:
        ...
        elif self._settings.inference_provider == "openai":
            await self._run_openai_inference(frame)
        else:
    ...
    async def _run_ollama_inference(self, frame: ProcessedFrame) -> None:
        ...
        payload = {
            "model": self._settings.model_name,
            "prompt": self._settings.inference_prompt,
            ...
            "messages": [
                {
                    ...
                    "content": [
                        ...
                        {
                            "type": "text",
                            "text": self._settings.inference_prompt
                        }
                    ]
                }
            ],
    ...
    async def _run_openai_inference(self, frame: ProcessedFrame) -> None:
        """运行 OpenAI 兼容推理.
        ...
        Args:
            frame: 处理后的帧
        """
        # OpenAI 兼容 API 使用 Chat Completions 格式
        payload = {
            "model": self._settings.openai_model,
            "messages": [
                {
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if choices:
                    ...
                    logger.warning("openai_empty_response", response=data)
            else:
                logger.warning(
                    "openai_http_error",
                    status=response.status_code,
